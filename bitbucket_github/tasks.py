...
def copy_to_github(user, repo_slug):
    print(f'Copying {repo_slug}...')

    # get profiles
    print('\tFetching profiles...')
    bitbucket_profile = requests.get(f'{BITBUCKET_ENDPOINT}/user?access_token=${user.bitbucket_token}').json()
    github_profile = requests.get(url=f'{GITHUB_ENDPOINT}/user', headers={'Authorization': f'token {user.github_token}'})

    # get repo
    print('\tFetching repo information...')
    repo = requests.get(f'{BITBUCKET_ENDPOINT}/repositories/{bitbucket_profile["username"]}/{repo_slug}?access_token=${user.bitbucket_token}').json()
    ...
    # clone
    print('\tCloning repo...')
    clone_link = repo['links']['clone'][0]['href'].replace(bitbucket_profile['username'], f'x-token-auth:{user.bitbucket_token}', 1)
    ...
    # push to new repo
    print('\tPushing code to Github repo...')
